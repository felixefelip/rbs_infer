  ...
  module ErbConvention
    class Generator
      ...
      def collect_helper_methods(view_info)
        ...
        # Always check ApplicationController
        app_ctrl = File.join(@app_dir, "app/controllers/application_controller.rb")
        if File.exist?(app_ctrl)
          methods.merge!(extract_helper_method_signatures(app_ctrl, "ApplicationController"))
        end

        # Check specific controller if it's not ApplicationController
        if view_info
